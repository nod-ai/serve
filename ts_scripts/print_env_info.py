...
def get_gpu_info():
    ...
    gpu_types = [torch.cuda.get_device_name(gpu_index) for gpu_index in range(num_of_gpus)]
    return "\n".join(["", *gpu_types])
...
def populate_hip_env(hip_available_str):
    ...
    hip_env["rocm_version"] = run_and_parse_first_match("amd-smi version", r"ROCm version: ([\d.]+)")
