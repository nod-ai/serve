...
import org.pytorch.serve.device.utils.XpuUtil;
...
public class XpuUtilTest {
  ...
  public void testGetGpuEnvVariableName() {
    assertEquals(xpuUtil.getGpuEnvVariableName(), "XPU_VISIBLE_DEVICES",
  ...
  public void testGetUtilizationSmiCommand() {
    ...
    assertArrayEquals(xpuUtil.getUtilizationSmiCommand(), expectedCommand,
  ...
  public void testSmiOutputToUpdatedAccelerators() {
    ...
    ArrayList<Accelerator> updatedAccelerators = xpuUtil.smiOutputToUpdatedAccelerators(smiOutput, parsedGpuIds);

    assertEquals(updatedAccelerators.size(), 2, "Should return 2 updated accelerators");
    assertEquals((int) updatedAccelerators.get(0).getAcceleratorId(), 0, "First accelerator should have ID 0");
    assertEquals((int) updatedAccelerators.get(1).getAcceleratorId(), 1, "Second accelerator should have ID 1");
    assertEquals((float) updatedAccelerators.get(0).getUsagePercentage(), 50.00f, 0.01,
        "GPU utilization should match for first accelerator");
    assertEquals((float) updatedAccelerators.get(0).getMemoryUtilizationPercentage(), 75.50f, 0.01,
        "Memory utilization should match for first accelerator");
    assertEquals((float) updatedAccelerators.get(1).getUsagePercentage(), 25.00f, 0.01,
        "GPU utilization should match for second accelerator");
    assertEquals((float) updatedAccelerators.get(1).getMemoryUtilizationPercentage(), 60.25f, 0.01,
  ...
  public void testSmiOutputToUpdatedAcceleratorsWithFilteredIds() {
    ...
    assertEquals(updatedAccelerators.size(), 2, "Should return 2 updated accelerators");
    assertEquals((int) updatedAccelerators.get(0).getAcceleratorId(), 0, "First accelerator should have ID 0");
    assertEquals((int) updatedAccelerators.get(1).getAcceleratorId(), 2, "Second accelerator should have ID 2");
    assertEquals((float) updatedAccelerators.get(0).getUsagePercentage(), 50.00f, 0.01,
        "GPU utilization should match for first accelerator");
    assertEquals((float) updatedAccelerators.get(0).getMemoryUtilizationPercentage(), 75.50f, 0.01,
        "Memory utilization should match for first accelerator");
    assertEquals((float) updatedAccelerators.get(1).getUsagePercentage(), 30.00f, 0.01,
        "GPU utilization should match for third accelerator");
    assertEquals((float) updatedAccelerators.get(1).getMemoryUtilizationPercentage(), 70.00f, 0.01,
  ...
  public void testSmiOutputToUpdatedAcceleratorsWithInvalidInput() {
    ...
    ArrayList<Accelerator> accelerators = xpuUtil.smiOutputToUpdatedAccelerators(invalidSmiOutput, parsedGpuIds);
